	...
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			...
			% % Sliko smo naredili tako, da so točke A, B, T in C vse enako oddaljene
				...
				\coordinate (A') at ({-\tt},{\td});
				\draw (A') arc[start angle=210, end angle=330, radius=2] node[right] {\scriptsize $k$};
				\pause
				% Točka A
				...
				\pause	\coordinate (T'') at ({1.7*\td},{-\tt});
						\draw[gray, thin] (T'') node[left] {\scriptsize $n$} arc[start angle=240, end angle=120, radius=2] ;
				% Naloga 3.4.4.: Definirajte in narišite točko C

				\pause	\coordinate [label= right:{\scriptsize $C$}] (C) at (0,{-0.75*\t});
						\draw (C) circle (1.5pt);
				% Naloga 3.4.5.: Narišite premico skozi točki T in C
				\pause	\draw[red,very thick] (0,-2) -- (0,2);
